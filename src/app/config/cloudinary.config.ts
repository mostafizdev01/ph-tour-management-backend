...
// from data er mordhe text gula thakbe && file er mordhe image gula thakbe
...
cloudinary.config({ // image upload configuratiion && cludinary info
...
export const uploadBufferToCloudinary = async (buffer: Buffer, fileName: string): Promise<UploadApiResponse | undefined> => {
    try {
        return new Promise((resolve, reject) => {
            ...
            const public_id = `pdf/${fileName}-${Date.now()}`  // create public_id
...
export const deleteImageFromCLoudinary = async (url: string) => {
    try {
        ...
        const regex = /\/v\d+\/(.*?)\.(jpg|jpeg|png|gif|webp)$/i;  // get public id

        const match = url.match(regex);

        console.log({ match });

        if (match && match[1]) { /// public id ta arr er secound index a ashbe
